...
import { sendMessageToAI, getMusicPrompts, resetChatSession, updateAIContext } from '../api/aiApi';
...
export default function ChatModal({ visible, onClose }: ChatModalProps) {
    ...
    // Get music context
    const { currentSong, songs, likedSongs, playlists } = useMusic();

    // Load chat history and update AI context when modal opens
    useEffect(() => {
        if (visible) {
            loadChatHistory();
            // Update AI with current app context
            updateAIContext({
                currentSong,
                songs,
                likedSongs,
    ...
    }, [visible, currentSong, songs, likedSongs, playlists]);
